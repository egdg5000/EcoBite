...
const RegistrationScreen = () => {
    ...
    const validate = () => {
        ...
        if (!username) {
          setErrorUsername('Vul uw gebruikersnaam in');
          error = true;
        } else {
          setErrorUsername('');
        }
        ...
        if (!newemail) {
          setErrorEmail('Vul uw mailadres in');
          error = true;
        } else {
          const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
          if (!emailRegex.test(newemail)) {
            setErrorEmail('Vul een geldig mailadres in');
            error = true;
          } else setErrorEmail('');
        }
    
        if (!password) {
          setErrorPassword('Vul uw wachtwoord in');
          error = true;
        } else {
    ...
    const throwError = (response: any) => {
        if (response.message === 'Gebruikersnaam en maildres zijn al in gebruik') {
          setErrorUsername('Gebruikersnaam is al in gebruik');
          setErrorEmail('Maildres is al in gebruik');
        };
        if (response.message === 'Gebruikersnaam is al in gebruik') setErrorUsername(response.message);
        if (response.message === 'Maildres is al in gebruik') setErrorEmail(response.message);
    }
